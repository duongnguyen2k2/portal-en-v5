...
var vmArticlesFestival = new Vue({
	el: '#articles_festival-app',
	data: {
		title: "Quản lý tour",
		detailItem: { Id: 0, Ids: '', Title: '', Alias: '', Introtext: '', Description: '', AuthorName: '', Status: 1 },
		deleteItem: { Id: 0, Ids: '', Title: '', Alias: '', Introtext: '', Description: '', AuthorName: '', Status: 1 },
		SearchItems: { CurrentPage: 1, ItemsPerPage: 10, Keyword: '', TotalItems: 0, PageCount: 0, Status: -1 },
	...
	mounted() {
		document.getElementById("articles_festival-app").style.display = "block";
	...
	methods: {
		...
		getListJson: function () {
			...
			$.ajax({
				type: "POST",
				url: '/Admin/ArticlesFestival/GetListJson',
				...
				success: resul => {
					...
					if (resul.Success) {
						...
						if (this.ListItems != null && this.ListItems.length > 0) {d
		...
		changeStatus: function (i) {
			...
			return $.ajax({
				type: "GET",
				url: '/Admin/ArticlesFestival/UpdateStatus?Ids=' + this.ListItems[i].Ids + "&Status=" + Status,
				headers: this.headers,
				contentType: "application/json; charset=utf-8",
				dataType: "json",
				success: resul => {
		...
		deletedItem: function () {
			return $.ajax({
				type: "POST",
				url: '/Admin/ArticlesFestival/DeleteItem',
				headers: this.headers,
		...
		saveItem: function () {
			if (this.checkForm()) {
				...
				return $.ajax({
					type: "POST",
					url: '/Admin/ArticlesFestival/SaveItemJson/',
					headers: this.headers,
