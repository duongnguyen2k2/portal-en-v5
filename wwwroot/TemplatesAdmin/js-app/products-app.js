...
var vmProducts = new Vue({
	...
	data: {
		...
		SearchItems: { CurrentPage: 1, ItemsPerPage: 30, Keyword: '', TotalItems: 0, PageCount: 0, Status: -1, CatId: -1, IdManufacturer: 0 },
		flagLoadingCat: false,
		flagLoadingCatManufacturer: false,
		flagLoading: false,
	...
	mounted() {
		document.getElementById("app-products").style.display = "block";
	...
	methods: {
		...
		changeFeatured: function (i) {
			...
			return $.ajax({
				type: "GET",
				url: '/Admin/Products/UpdateFeatured?Ids=' + this.ListItems[i].Ids + "&Featured=" + Featured,
		...
		changeStatus: function (i) {
			...
			return $.ajax({
				type: "GET",
				url: '/Admin/Products/UpdateStatus?Ids=' + this.ListItems[i].Ids + "&Status=" + Status,
				headers: this.headers,
				contentType: "application/json; charset=utf-8",
				dataType: "json",
				success: resul => {
		...
		saveItem: function () {
			if (this.checkForm()) {
				...
				return $.ajax({
					type: "POST",
					url: '/Admin/Products/SaveItemJson/',
					headers: this.headers,
					data: JSON.stringify(this.detailItem),
					contentType: "application/json; charset=utf-8",
					dataType: "json",
		...
		getListJson: function () {
			...
			$.ajax({
				type: "POST",
				url: '/Admin/Products/GetListJson',
				headers: this.headers,
		...
		getListCatManufacturer: function () {
			this.flagLoadingCatManufacturer = true;
			$.ajax({
				type: "GET",
				url: '/Admin/Manufacturer/GetListSelectItem',
				headers: this.headers,
				data: "json",
				contentType: "application/json; charset=utf-8",
				dataType: "json",
				success: resul => {
					this.flagLoadingCatManufacturer = false;
					if (resul.Success) {
						console.log(resul.Data);
						this.ListCatManufacturer = resul.Data;
					}
				},
				error: function () {
		...
		openArticlesProduct: function (i) {
			console.log("openArticlesProduct: " + i);
			var ids = this.ListItems[i].Ids;
			var idManu = this.ListItems[i].IdManufacturer;
			window.location = '/Admin/ArticlesProduct?Ids=' + ids + '&IdManufacturer=' + idManu;
		},
		...
		showDeleted: function (i) {
			console.log("showDeleted", this.ListItems[i]);
			this.resetFormDetail();
